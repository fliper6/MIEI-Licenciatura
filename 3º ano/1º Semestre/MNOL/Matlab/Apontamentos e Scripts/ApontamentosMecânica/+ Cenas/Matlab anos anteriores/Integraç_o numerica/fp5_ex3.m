...
pause;
disp('      ');
disp('|e(Isc)| <= M4 *(b-a)^5 /(180 n^4) <= tol, M4 = max |f^4(x)| em [1, 2], então');
disp('o número mínimo de subintervalos n que garante |e(Isc)| <= tol');
disp(' é dado por n >= raiz quarta de (M4*(b-a)^4/(180*tol)');
disp('               ');
disp('1º. Calcular M4 = max |f^4(x)| em [1, 2]');
...
y1= abs(subs(df4,1));
y2 =abs(subs(df4,2));
disp(['|f^4(1)|=', num2str(y1, 4), ', |f^4(2)|=', num2str(y2, 4), ' então M4 =max |f^4(x)|=|f^4(1)| =', num2str(y1, 4)]);
...
disp('o número mínimo de subintervalos n que garante |e(Isc)| <= tol');
disp('é dado pelo menor valor inteiro positivo PAR que garante que');
...
%------------------------------------------------------------------
%  Gladys Castillo, António Pereira, U.A, 2010
%------------------------------------------------------------------
