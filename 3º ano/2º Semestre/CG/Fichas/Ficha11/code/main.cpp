...
void prepareCylinder(float altura, float raio, int lados) {
	...
	float* v, * n, * t;
	float tess = 1.0 / lados;
	float raioTextura = 0.1875;
	...
	float centroTop[2] = { 0.4375,0.1875 };
	float centroBot[2] = { 0.8125,0.1875 };

	for (int i = 0; i < lados; ++i) {
		// topo
		// ponto central
		t[vertex * 2 + 0] = centroTop[0];
		t[vertex * 2 + 1] = centroTop[1];
		n[vertex * 3 + 0] = 0.0f;
		n[vertex * 3 + 1] = 1.0f;
		n[vertex * 3 + 2] = 0.0f;
		v[vertex * 3 + 0] = 0.0f;
		...
		vertex++;
		t[vertex * 2 + 0] = centroTop[0] + raioTextura * sin(i * delta);
		t[vertex * 2 + 1] = centroTop[1] + raioTextura * cos(i * delta);
		n[vertex * 3 + 0] = 0;
		n[vertex * 3 + 1] = 1.0f;
		n[vertex * 3 + 2] = 0;
		...
		vertex++;
		t[vertex * 2 + 0] = centroTop[0] + raioTextura * sin((i + 1) * delta);
		t[vertex * 2 + 1] = centroTop[1] + raioTextura * cos((i + 1) * delta);
		n[vertex * 3 + 0] = 0;
		n[vertex * 3 + 1] = 1.0f;
		n[vertex * 3 + 2] = 0;
		...
		vertex++;
		t[vertex * 2 + 0] = centroBot[0];
		t[vertex * 2 + 1] = centroBot[1];
		n[vertex * 3 + 0] = 0.0f;
		n[vertex * 3 + 1] = -1.0f;
		n[vertex * 3 + 2] = 0.0f;
		v[vertex * 3 + 0] = 0.0f;
		...
		vertex++;
		t[vertex * 2 + 0] = centroBot[0] + raioTextura * sin((i + 1) * delta);
		t[vertex * 2 + 1] = centroBot[1] + raioTextura * cos((i + 1) * delta);
		n[vertex * 3 + 0] = 0.0f;
		n[vertex * 3 + 1] = -1.0f;
		n[vertex * 3 + 2] = 0.0f;
		v[vertex * 3 + 0] = raio * sin((i + 1) * delta);
		...
		vertex++;
		t[vertex * 2 + 0] = centroBot[0] + raioTextura * sin(i * delta);
		t[vertex * 2 + 1] = centroBot[1] + raioTextura * cos(i * delta);
		n[vertex * 3 + 0] = 0.0f;
		n[vertex * 3 + 1] = -1.0f;
		n[vertex * 3 + 2] = 0.0f;
		v[vertex * 3 + 0] = raio * sin(i * delta);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 2 * vertexCount, t, GL_STATIC_DRAW);
